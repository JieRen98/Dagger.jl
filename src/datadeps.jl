...
function has_writedep(state::DataDepsState{DataDepsAliasingState}, arg, deps)
...
datadeps region.
"""
function has_writedep(state::DataDepsState, arg, deps, task::EagerThunk)
...
function populate_argument_info!(state::DataDepsState{DataDepsAliasingState}, arg, deps)
    ...
    for (dep_mod, readdep, writedep) in deps
        ...
        if !haskey(astate.ainfos_owner, ainfo)
            ...
            astate.ainfos_readers[ainfo] = Pair{EagerThunk,Int}[]
        end
...
function populate_argument_info!(state::DataDepsState{DataDepsNonAliasingState}, arg, deps)
    ...
    if !haskey(astate.args_owner, arg)
        ...
        astate.args_readers[arg] = EagerThunk[]
    end
...
function get_write_deps!(state::DataDepsState{DataDepsAliasingState}, ainfo::AbstractAliasing, task, write_num, syncdeps)
    astate = state.alias_state
...
function get_read_deps!(state::DataDepsState{DataDepsAliasingState}, ainfo::AbstractAliasing, task, write_num, syncdeps)
    astate = state.alias_state
    ainfo isa NoAliasing && return
    for other_ainfo in astate.ainfos_overlaps[ainfo]
...
function get_write_deps!(state::DataDepsState{DataDepsNonAliasingState}, arg, syncdeps)
    @warn "Learn about write_num" maxlog=1
    if (owner = state.alias_state.args_owner[arg]) !== nothing
        push!(syncdeps, owner)
    end
    get_read_deps!(state, arg, syncdeps)
end
function get_read_deps!(state::DataDepsState{DataDepsNonAliasingState}, arg, syncdeps)
    for reader in state.alias_state.args_readers[arg]
        push!(syncdeps, reader)
    end
end
function add_writer!(state::DataDepsState{DataDepsNonAliasingState}, arg, task)
    state.alias_state.args_owner[arg] = task
    empty!(state.alias_state.args_readers[arg])
    # Not necessary to assert a read, but conceptually it's true
    add_reader!(state, arg, task)
end
function add_reader!(state::DataDepsState{DataDepsNonAliasingState}, arg, task)
    push!(state.alias_state.args_readers[arg], task)
end
        ...
        for (idx, (pos, arg)) in enumerate(task_args)
            ...
            if queue.aliasing
                ...
                if nonlocal
                    ...
                    copy_to_scope = our_scope
                    copy_to_syncdeps = Set{Any}()
                    get_write_deps!(state, arg, copy_to_syncdeps)
        ...
        for (idx, (_, arg)) in enumerate(task_args)
            ...
            if queue.aliasing
                ...
                if is_writedep(arg, deps, task)
                    ...
                    get_write_deps!(state, arg, syncdeps)
                else
                    get_read_deps!(state, arg, syncdeps)
                end
            end
    ...
    if queue.aliasing
        ...
        for arg in keys(astate.data_origin)
            ...
            if data_local_space != data_remote_space
                ...
                copy_from_syncdeps = Set()
                get_write_deps!(state, arg, copy_from_syncdeps)
                @dagdebug nothing :spawn_datadeps "$(length(copy_from_syncdeps)) syncdeps"
